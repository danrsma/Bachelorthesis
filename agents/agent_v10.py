...
async def main():
    ...
    if file_path != "":
        ...
        for i in range(10):
            ...
            result_loop1 = chain.invoke(
                {"text": "How does image_loop compare to image? What are the differences?", "image": image_b64, "image_type": image_type, "image_loop": image_b64_loop, "image_type_loop": "png"}
            )
            print("\n")
            print("ImageLLM Output:")
            print("\n")
            print(result_loop1)
            print("\n")
            result_loop2 = llm2.invoke(
                
                str(result_code)+"Image 2 is the result of the provided Blender Code.\n"+str(result_loop1)+"\nRewrite the Blender Code to make the differences smaller."
            )
            print("\n")
            print("CodeLLM Output:")
            print("\n")
            ...
            #3. MCP Action
            try:
                result_loop3 = await agent.ainvoke(
                    {"messages": [{"role": "user", "content":  "execute_code\n"+result_loop2.content+"\nIf it does not work try to fix and reexecute it."}]}
                )
        ...
        result2 = llm2.invoke(
            str(user_input)+""" Create Blender Code for the described Landscape. Create every Object and Shape with math.Put the following lines at the end: 
            bpy.context.scene.render.filepath = "/home/daniel/Bachelorarbeit/agents/render.png"
            bpy.ops.render.render(write_still=True)
            Do not use the following.
            bpy.ops.object.camera_add()
        ...
        for i in range(10):
            ...
            result_loop1 = chain.invoke(
                {"text": "How does image compare to the the discription:"+str(user_input)+"? What are the differences?", "image": image_b64_loop, "image_type": "png"}
            )
            print("\n")
            print("ImageLLM Output:")
            print("\n")
