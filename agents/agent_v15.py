...
async def main():
    ...
    if file_path != "":
        ...
        file_path_loop = "/home/student-rossmaier/Bachelorarbeit/agents/render.png"
        ...
        # Create StateGraph With Nodes And Edges Without Vision
        graph_no_vi = StateGraph(MyState)
        graph_no_vi.add_node("code_llm", code_llm_func)
        graph_no_vi.add_node("tools_llm", tools_llm_func)
        graph_no_vi.add_edge(START,"code_llm")
        graph_no_vi.add_edge("code_llm", "tools_llm")
        graph_no_vi.add_edge("tools_llm",END)
        graph = graph.compile()
        
        # Get StateGraph Output State Without Vision
        prompt_code = "Create Blender Code of the described Landscape. Create every Object and Shape with math."    
        input_state = MyState(userinput=user_input,filepath=file_path,promptcode=prompt_code,promptvision="",code="")
        output_state = await graph.ainvoke(input_state)
        print(output_state)
        ...
        prompt_code_loop = """The new image is the result of the provided Blender Code. 
            ...
            """
        output_state["filepath"] = file_path_loop
        output_state["promptvision"] = user_input+prompt_vision_loop
        output_state["promptcode"] = prompt_code_loop
